...
class RhythmGameUI:
    ...
    def create_ui_elements(self):
        ...
        self.start_button = Button(
            SCREEN_WIDTH // 2 - 150,  # Center it under the Song Library panel
            700,  # Position it below the Song Library panel
            250,  # Reduced width from 300 to 250
            50,   # Reduced height from 60 to 50
            "START GAME", 
            color=ACCENT_COLOR,
    ...
    def draw_menu(self):
        ...
        # Draw song library section with modern panel
        self.draw_panel(SCREEN_WIDTH // 2 - 350, 470, 700, 220, "Song Library")
        
        # Draw song list with better spacing and styling
        y_offset = 520
        max_songs_visible = 6  # Limit the number of songs visible
        song_height = 30  # Reduced height per song

        for i, (song, bpm) in enumerate(sorted(SONGS.items(), key=lambda x: x[1])[:max_songs_visible]):
            difficulty = get_difficulty_name(bpm)
            
            # Draw song name with difficulty indicator - use smaller font
            diff_color = SUCCESS_COLOR if difficulty == "Easy" else ACCENT_COLOR
            
            # Truncate song name if too long
            song_display = song
            if len(song) > 20:
                song_display = song[:18] + ".."
            
            song_surf = SMALL_FONT.render(song_display, True, LIGHT_TEXT)
            self.screen.blit(song_surf, (SCREEN_WIDTH // 2 - 300, y_offset))
            
            # BPM and difficulty
            bpm_text = f"{bpm} BPM"
            bpm_surf = SMALL_FONT.render(bpm_text, True, GRAY)
            self.screen.blit(bpm_surf, (SCREEN_WIDTH // 2 + 50, y_offset))
            
            # Difficulty indicator
            diff_surf = SMALL_FONT.render(difficulty, True, diff_color)
            diff_rect = diff_surf.get_rect(right=SCREEN_WIDTH // 2 + 300)
            diff_rect.y = y_offset
            self.screen.blit(diff_surf, diff_rect)
            
            y_offset += song_height  # Reduced spacing between songs
    ...
    def draw_game(self):
        ...
        if self.game and self.game.is_tracking:
            ...
            # Draw "CLAP!" text with modern styling
            if self.show_beat:
                clap_surf = TITLE_FONT.render("CLAP!", True, WHITE)
                clap_rect = clap_surf.get_rect(center=(SCREEN_WIDTH // 2, SCREEN_HEIGHT // 2))
                
                # Text shadow for better readability
                shadow_surf = TITLE_FONT.render("CLAP!", True, (0, 0, 0))
    ...
    def draw_results(self):
        ...
        if self.score_display:
            ...
            # Draw winner with special styling
    ...
    def initialize_game(self):
        ...
        if self.input_mode == "keyboard":
            ...
            # Create a dummy audio handler that            # Create a dummy audio handler that won't be used
